...
const RoleBadge = ({
  ...
  if (role === 'owner') {
    return (
      <View className="flex-row items-center rounded-full bg-[#FFF4D8] px-2.5 py-1">
        ...
        <Text className="ml-1 font-body text-[10px] font-bold text-[#96620F]">
          Owner
        </Text>
      </View>
    );
  ...
  if (role === 'admin') {
    return (
      <View className="flex-row items-center rounded-full bg-[#EEE9FF] px-2.5 py-1">
        ...
        <Text className="ml-1 font-body text-[10px] font-bold text-[#6D28D9]">
          Admin
        </Text>
      </View>
    );
  ...
  return (
    <View className="rounded-full bg-[#F3F1EF] px-2.5 py-1">
      <Text className="font-body text-[10px] font-semibold text-[#77716D]">
        Member
      </Text>
    </View>
  );
...
export default function GroupInfoScreen() {
  ...
  const [
    ...
    useState<PickedGroupImage | null>(
      null,
    );
  ...
  const updateGroupInfo =
    useMutation(
      api.chat.groupInfo.updateGroupInfo,
    );

  const generateGroupImageUploadUrl =
    useMutation(
      api.chat.groupInfo
        .generateGroupImageUploadUrl,
    );

  const cleanupUnusedGroupImage =
    useMutation(
      api.chat.groupInfo
        .cleanupUnusedGroupImage,
    );

  const addMembers =
    useMutation(
      api.chat.groupInfo.addMembers,
    );

  const removeMember =
    useMutation(
      api.chat.groupInfo.removeMember,
    );

  const leaveGroup =
    useMutation(
      api.chat.groupInfo.leaveGroup,
    );

  const deactivateGroup =
    useMutation(
      api.chat.groupInfo.deactivateGroup,
    );
  ...
  const uploadSelectedImage =
    async () => {
      ...
      const uploadUrl =
        await generateGroupImageUploadUrl(
          {
            groupId,
          },
        );
      ...
      if (
        ...
        throw new Error(
          'Group image cannot exceed 5 MB.',
        );
      }
      ...
      if (!result.storageId) {
        throw new Error(
          'The image upload did not return a storage ID.',
        );
      }
  ...
  const handleSaveGroup =
    async () => {
      ...
      try {
        ...
        Alert.alert(
          'Group updated',
          'The group information was saved.',
        );
      } catch (error) {
  ...
  const handleAddMembers =
    async () => {
      ...
      if (
        selectedUserIds.length > 100
      ) {
        Alert.alert(
          'Too many members',
          'Add no more than 100 members at once.',
        );
  ...
  const confirmLeaveGroup = () => {
    ...
    Alert.alert(
      'Leave group?',
      `You will no longer receive messages from ${groupInfo.name}.`,
  ...
  if (
    ...
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-[#FAF8F6]">
        ...
        <Text className="mt-4 font-body text-sm text-[#77716D]">
      ...
      <ScrollView
            ...
            <View className="mt-6 flex-row">
              ...
              {groupInfo.canManageMembers ? (
                <TouchableOpacity
                  ...
                  <Text className="ml-2 font-body text-sm font-bold text-white">
                    Add members
                  </Text>
                </TouchableOpacity>
              ) : null}
        ...
        <View style={styles.sectionCard}>
          <View className="flex-row items-center justify-between">
            <Text className="font-heading text-lg font-bold text-[#1A1A1A]">
              Members
            </Text>
          ...
          <View className="mt-2">
                  ...
                  <View
                    ...
                    <View className="ml-3 min-w-0 flex-1">
                      ...
                      {member.email ? (
                        <Text
                          numberOfLines={1}
                          className="mt-0.5 font-body text-xs text-[#77716D]">
                          {member.email}
                        </Text>
                      ) : null}
            ...
            0 ? (
              <View className="items-center py-8">
                ...
                <Text className="mt-2 font-body text-sm text-[#77716D]">
                  No members found
                </Text>
              </View>
            ) : null}
        ...
        <View style={styles.sectionCard}>
          ...
          {groupInfo.canLeaveGroup ? (
            <TouchableOpacity
              ...
              <View className="ml-3 flex-1">
                <Text className="font-body text-sm font-bold text-[#D95A14]">
                  Leave group
                </Text>
            ...
            <View className="mt-4 rounded-2xl bg-[#F7F3F0] px-4 py-3">
              <Text className="font-body text-xs leading-5 text-[#77716D]">
                Group owners cannot leave their group. Delete the group or transfer ownership before leaving.
              </Text>
            </View>
          )}
          ...
          {groupInfo.canDeleteGroup ? (
            <TouchableOpacity
              ...
              <View className="ml-3 flex-1">
                <Text className="font-body text-sm font-bold text-[#D92D20]">
                  Delete group
                </Text>
        ...
        <SafeAreaView className="flex-1 bg-white">
          ...
          undefined ? (
            <View className="flex-1 items-center justify-center">
              ...
              <Text className="mt-3 font-body text-sm text-[#77716D]">
                Loading people…
              </Text>
            </View>
          ) : (
