...
export default function AddNewActivity() {
  ...
  const addHealthDataManually = useMutation(api.activities.addHealthDataManually);
  const generrateUploadUrl = useMutation(api.upload.generateUploadUrl);
  const today = useMemo(() => new Date(), []);
  const adjustedDate = useMemo(
    () => getDateForTimezone(today, Intl.DateTimeFormat().resolvedOptions().timeZone),
    [today]
  );
  const [selectedDate, setSelectedDate] = useState(adjustedDate);
  ...
  const onChange = (event: any, newDate: any) => {
    if (newDate) {
      // Adjust the newly selected date to the specified timezone
      const adjustedNewDate = getDateForTimezone(
        newDate,
        Intl.DateTimeFormat().resolvedOptions().timeZone
      );
      setSelectedDate(adjustedNewDate);
    }
  };
          ...
          <View className="flex-1 justify-start px-6">
            ...
            <View className="mt-4">
              ...
              <View className="mb-2 flex-col items-center justify-center gap-y-2">
                <Text className="text-center text-xl font-bold text-black">Add movement for </Text>
                {Platform.OS === 'android' ? (
                  <>
                    <TouchableOpacity
                      onPress={() =>
                        DateTimePickerAndroid.open({
                          ...
                          minimumDate: startOfCurrentMonth,
                        })
                      }>
                      <Text className="rounded-lg bg-gray-200 px-2 py-1 text-sm font-bold text-black">
                        {selectedDate.toLocaleDateString('en-GB', {
                          day: 'numeric',
                          month: 'short',
                          year: 'numeric',
                          timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone, // Use the specified timezone
                        })}
                      </Text>
                    </TouchableOpacity>
                  </>
              ...
              <View className="flex-1">
                <ButtonGroup>
                  <LoadingButton
                    ...
                    loading={isLoading}>
                    <ButtonText>Submit Activity</ButtonText>
                  </LoadingButton>
                </ButtonGroup>
              </View>
            </View>
