...
async function validateGroupImage(
  ctx: MutationCtx,
...
export const listGroupsForAdmin = query({
  ...
  handler: async (ctx) => {
    ...
    const groups = (
      await ctx.db.query("chatGroups").order("desc").take(100)
    ...
    return await Promise.all(
      groups.map(async (group) => {
        ...
        return {
          ...
          imageUrl: group.imageStorageId
            ? await ctx.storage.getUrl(group.imageStorageId)
            : null,
...
export const createGroup = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.memberIds.length > MAX_MEMBERS_PER_MUTATION) {
      throw new ConvexError(
        `You can add at most ${MAX_MEMBERS_PER_MUTATION} members at once`,
      );
    }
...
export const updateGroup = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.imageStorageId && args.removeImage) {
      throw new ConvexError(
        "A new image and image removal cannot be requested together",
      );
    }
